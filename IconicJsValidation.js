...
var IconicJsValidation = (function () {
    function IconicJsValidation() {
    }
    return IconicJsValidation;
}());
...
var IconicJsValidationRule = (function () {
    ...
    IconicJsValidationRule.prototype.addRule = function (rulename, modifier, message) {
        ...
        if (this.Debug) {
            console.groupCollapsed("Added rule: " + this.getRuleText(rulename, modifier, message));
            var keys = Object.keys(this.Rules);
            var length_1 = keys.length;
            console.group("Current rules:");
            for (var x = 0; x < length_1; x++) {
                console.log(x + ") " + this.getRuleText(keys[x], this.Rules[keys[x]], this.Messages[keys[x]]));
            }
            console.groupEnd();
            console.groupEnd();
        }
    };
    IconicJsValidationRule.prototype.getRuleText = function (rulename, modifier, message) {
        return rulename + "(" + modifier + "): " + message;
    };
